...
class SymcaToolBox(object):
    ...
    def get_es_matrix(mod, nmatrix, fluxes, species):
        ...
        for col in range(nmat.cols):
            ...
            for row in range(nmat.rows):
                ...
                cond1 = str(current_species) in getattr(mod, str(current_reaction)[2:]).symbols
                cond2 = str(current_species) in mod.species

                if cond1 and cond2:
                    elas_row.append('ec' + str(current_reaction)[2:] + '_' + str(current_species))
                else:
                    elas_row.append(0)
    ...
    def det_bareis(matrix):
        """
        Adapted from original det_bareis function in Sympy 0.7.1.
        cancel() and expand() are removed from function to speed
        up calculations. Maxima will be used to speed simplify
        the result
